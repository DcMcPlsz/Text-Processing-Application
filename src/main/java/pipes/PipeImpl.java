...
public class PipeImpl<T> implements Pipe<T> {
    ...
    // not using an exception because would require consumers to write unidiomatic
    //
    // `while(true)`
    // not using an Option because there is no standard Option and reimplementing it
    //
    // is too annoying
